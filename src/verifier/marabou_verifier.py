...
class MarabouVerifier(Verifier):
    ...
    def _solver_solve(self, solver, fml):
        results =  [quadrant.solve(options=solver, verbose=False)[0] for quadrant in fml]   # List of dicts of counter examples for each quadrant
        combined_results = {}
        for result in results:
            combined_results.update(result) # This means counterexamples are lost - not a permanent solution
    ...
    def domain_constraints(self, V: Marabou.MarabouNetworkONNX, Vdot: Marabou.MarabouNetworkONNX):
        """
        :param V:
        :param Vdot:
        ...
        Vdot_tuple = tuple(deepcopy(Vdot) for i in range(len(orthants))) # Definitely bad usage of memory - a generator would be better
        ...
        for i, orthant in enumerate(orthants):
            ...
            for j, var in enumerate(V_input_vars):
                V_tuple[i].setLowerBound(var, min(orthant[j] * inner, orthant[j] * outer))
                V_tuple[i].setUpperBound(var, max(orthant[j] * inner, orthant[j] * outer))
                Vdot_tuple[i].setLowerBound(var, min(orthant[j] * inner, orthant[j] * outer))
                Vdot_tuple[i].setUpperBound(var, max(orthant[j] * inner, orthant[j] * outer))
