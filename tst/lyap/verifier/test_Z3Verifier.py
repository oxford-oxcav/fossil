...
class TestZ3Verifier(unittest.TestCase):
    def test_poly2_with_good_Lyapunov_function(self):
        ...
        lc = certificate.Lyapunov(domains={"lie-&-pos": domain_z3})
        ver = verifier.VerifierZ3(n_vars, lc.get_constraints, var_bounds, x)

        # model
        model = learner.LearnerCT(
            2, None, 2, bias=False, activate=[ActivationType.SQUARE], equilibria=None
        )
        with torch.no_grad():
        ...
        tr = translator.TranslatorCT(
            model, np.array(x).reshape(-1, 1), np.array(xdot).reshape(-1, 1), None, 1
        )
    ...
    def test_poly2_with_bad_Lyapunov_function(self):
        ...
        # model
        model = learner.LearnerCT(
            2, None, 2, bias=True, activate=[ActivationType.SQUARE], equilibria=None
        )
