...
class ReluNet(torch.nn.Module):
    def __init__(self, c: float) -> None:
        ...
        self.model = torch.nn.Sequential(torch.nn.Linear(1, 2, bias=False),
                                         torch.nn.ReLU(), torch.nn.Linear(2, 1, bias=False))
        with torch.no_grad():
...
class MarabouVerifierTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        torch.onnx.export(net_pos, dummy_input, self.net_pos_file, input_names=['S', 'Sdot'],
                          output_names=['V'])
        torch.onnx.export(net_neg, dummy_input, self.net_neg_file, input_names=['S', 'Sdot'],
                          output_names=['V'])
        return super().setUp()
