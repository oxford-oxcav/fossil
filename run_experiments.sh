...
python3 -m experiments.benchmarks.stabsafe.stablesafe2 --record --repeat $N_EXP 
python3 -m experiments.benchmarks.stabsafe.ctrl_stablesafe1 --record --repeat $N_EXP 
python3 -m experiments.benchmarks.stabsafe.ctrl_stablesafe3 --record --repeat $N_EXP 
...
python3 -m experiments.benchmarks.rwa.ctrl_rwa_2_complex --record --repeat $N_EXP 
python3 -m experiments.benchmarks.rwa.ctrl_rwa_4 --record --repeat $N_EXP  # I thought I'd lost this benchmark, so rwa_2_complex was meant to replace it. I did it differently, and I'm not sure if both are right or if one is better, so will work out later
...
python3 -m experiments.benchmarks.rar.rar1 --record --repeat $N_EXP 
python3 -m experiments.benchmarks.rar.ctrl_rar1 --record --repeat $N_EXP 
